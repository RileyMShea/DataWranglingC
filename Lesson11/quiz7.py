...
def key_type(element, keys):
    ...
    if element.tag == "user":
        ...
        if re.search(lower, tag_key) or re.search(lower, tag_value):
            keys['lower'] += 1
        elif re.search(lower_colon, tag_key) or re.search(lower_colon, tag_value):
            keys['lower_colon'] += 1
        elif re.search(problemchars, tag_key) or re.search(problemchars, tag_value):
            keys['problemchars'] += 1
        else:
            keys['other'] += 1
    return keys
